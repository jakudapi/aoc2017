...
case class Velocity(dx: Long, dy: Long, dz: Long)
case class Acceleration(d2x: Long, d2y: Long, d2z: Long)

case class Particle(p: Position, v: Velocity, a: Acceleration) {
  def update(n: Long = 1): Unit = {
    p.x = p.x + arithmeticSum(n, v.dx, a.d2x)
    p.y = p.y + arithmeticSum(n, v.dy, a.d2y)
    p.z = p.z + arithmeticSum(n, v.dz, a.d2z)
  }
  ...
  val input: Array[Particle] = Source
    .fromFile(inputFile)
    .getLines
    .toArray
    .map(convert)
...
println(input(0))
  input.map(_.update(10000000L))
  val results = input.map{particle => abs(particle.p.x) +  abs(particle.p.y) +  abs(particle.p.z)}
  println(results.indexOf(results.min))
