 ...
 */
  ...
  val startTime = System.nanoTime()
    ...
    val FullLength = 256
    val Input =
      Array(97, 167, 54, 178, 2, 11, 209, 174, 119, 248, 254, 0, 255, 1, 64, 190)

    val TestInput = Array(3, 4, 1, 5)

    var testnums: Array[Int] = Array(0, 1, 2, 3, 4)
    ...
    for (length <- Input) {
      if (current + length >= FullLength) {
        ...
        nums = nums.slice(0, current) ++ reversedSegment ++ nums.slice(current + length, FullLength)

      }

      current = current + length + skip
      if (current >= FullLength) current = current - FullLength //wrap around
      skip = skip + 1
    }
